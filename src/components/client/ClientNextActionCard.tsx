...
import { ArrowRight, CheckCircle, ClockCountdown, NotePencil, Warning } from "@phosphor-icons/react/dist/ssr"

interface ClientNextActionCardProps {
  action: {
    type: "briefing" | "approval" | "task_overdue" | "task" | "new_update" | "default"
    title: string
    description: string
    href: string
    projectName?: string
...
export async function ClientNextActionCard({ action }: ClientNextActionCardProps): Promise<React.JSX.Element> {
        ...
        <div className="flex gap-6">
          ...
          <div className="flex flex-col gap-1">
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-brand-primary/60">
              {action.projectName ? `${action.projectName} • ` : ""}{t(`type.${action.type}`)}
            </p>
