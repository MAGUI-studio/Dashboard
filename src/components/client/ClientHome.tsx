...
import { ArrowRight, FolderOpen, Files, ClockCountdown, NotePencil } from "@phosphor-icons/react/dist/ssr"
...
export async function ClientHome({ userName, data, isBriefingEmpty }: ClientHomeProps): Promise<React.JSX.Element> {
  ...
  let nextAction: React.ComponentProps<typeof ClientNextActionCard>["action"] = {
    ...
    href: (activeProject ? `/projects/${activeProject.id}` : "/projects") as any,
  }
  ...
  if (isBriefingEmpty && activeProject) {
    nextAction = {
      ...
      href: `/projects/${activeProject.id}/briefing` as any,
    ...
    nextAction = {
      ...
      href: `/projects/${approval.projectId}/approvals` as any,
    ...
    nextAction = {
      ...
      href: `/projects/${task.projectId}/tasks` as any,
      projectName: task.project.name
    }
  }

  const heroStatus: React.ComponentProps<typeof ClientHeroStatus>["status"] = 
    isBriefingEmpty ? "briefing_incomplete" :
    data.pendingApprovals.length > 0 ? "awaiting_approval" :
    data.pendingTasks.length > 0 ? "need_shipment" : "on_track"
    ...
    <div className="flex w-full flex-col gap-10 lg:gap-14">
      <header className="flex flex-col justify-between gap-8 border-b border-border/20 pb-10 md:flex-row md:items-end">
        ...
        <div className="flex flex-wrap gap-3">
          <Button asChild variant="outline" className="rounded-full px-6 text-[10px] font-black uppercase tracking-[0.2em]">
            <Link href="/projects">
          ...
          <div className="flex flex-col gap-8">
             <h2 className="font-heading text-2xl font-black uppercase tracking-tight px-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: t("links.files"), href: (activeProject ? `/projects/${activeProject.id}/files` : "/projects") as any, icon: Files },
                  { label: t("links.briefing"), href: (activeProject ? `/projects/${activeProject.id}/briefing` : "/projects") as any, icon: NotePencil },
                  { label: t("links.timeline"), href: (activeProject ? `/projects/${activeProject.id}/timeline` : "/projects") as any, icon: FolderOpen },
                  { label: t("links.tasks"), href: (activeProject ? `/projects/${activeProject.id}/tasks` : "/projects") as any, icon: ClockCountdown },
                ].map((link) => (
                  <Button
                    key={link.label}
                    asChild
                  ...
                  >
                    <Link href={link.href}>
                      <link.icon className="size-6 text-brand-primary/60" weight="duotone" />
