...
import { CheckCircle, WarningCircle, ClockCountdown } from "@phosphor-icons/react/dist/ssr"

interface ClientApprovalHistoryProps {
  updates: DashboardUpdate[]
}

export async function ClientApprovalHistory({ updates }: ClientApprovalHistoryProps) {
  const t = await getTranslations("Approvals")
  
  const historyUpdates = updates.filter(u => u.requiresApproval && u.approvalStatus !== ApprovalStatus.PENDING)
        ...
        {historyUpdates.map((update) => (
          ...
          >
            <div className="flex items-center gap-5 overflow-hidden">
              <div className={`flex size-10 shrink-0 items-center justify-center rounded-xl bg-background/40 shadow-sm ring-1 ring-border/20 ${
                update.approvalStatus === ApprovalStatus.APPROVED ? "text-emerald-500" : "text-red-500"
              }`}>
