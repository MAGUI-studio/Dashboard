...
import { ArrowRight, Clock, CheckCircle, WarningCircle } from "@phosphor-icons/react/dist/ssr"
...
export function ClientTaskList({ tasks }: ClientTaskListProps) {
  const t = useTranslations("Dashboard.client_home.activity")
  ...
  return (
    <div className="grid gap-6">
      {tasks.map((task) => {
        ...
        return (
          <Card key={task.id} className="rounded-[2rem] border-border/40 bg-muted/5 transition-all hover:bg-muted/10">
            <CardContent className="flex flex-col gap-6 p-8 md:flex-row md:items-center md:justify-between lg:p-10">
              <div className="flex gap-6">
                <div className={`flex size-14 shrink-0 items-center justify-center rounded-2xl ${
                  isOverdue ? "bg-amber-500/10 text-amber-600" : "bg-brand-primary/10 text-brand-primary"
                }`}>
                  {isOverdue ? <WarningCircle weight="duotone" className="size-7" /> : <Clock weight="duotone" className="size-7" />}
                </div>
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-3">
                    ...
                    {isOverdue && (
                      <Badge variant="outline" className="h-5 rounded-full border-amber-500/30 bg-amber-500/5 px-2 text-[8px] font-black uppercase tracking-widest text-amber-600">
                        Atrasado
                      </Badge>
                    )}
                  </div>
                  ...
                  {task.dueDate && (
                    <div className="mt-2 flex items-center gap-2">
                      <Clock weight="bold" className={`size-3.5 ${isOverdue ? "text-amber-500/60" : "text-brand-primary/60"}`} />
                      <span className={`text-[10px] font-bold uppercase tracking-widest ${isOverdue ? "text-amber-600/80" : "text-brand-primary/80"}`}>
