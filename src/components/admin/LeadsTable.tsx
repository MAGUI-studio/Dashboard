...
export function LeadsTable({ leads }: LeadsTableProps): React.JSX.Element {
  ...
  const handleStatusUpdate = async (id: string, status: LeadStatus): Promise<void> => {
  ...
  const getStatusBadge = (status: LeadStatus): React.JSX.Element => {
    const variants: Record<LeadStatus, string> = {
      ...
      CONTATO_REALIZADO: 'bg-purple-500/10 text-purple-500 border-purple-500/20',
    ...
    return (
      <Badge variant="outline" className={`rounded-full px-3 py-1 font-medium ${variants[status]}`}>
        {t(`status.${status}`)}
      </Badge>
    )
  }
        ...
        <Table>
          ...
          <TableBody>
            {filteredAndSortedLeads.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="h-48 text-center text-xs font-bold uppercase tracking-widest text-muted-foreground/40">
                  {t('table.empty')}
                </TableCell>
              </TableRow>
            ) : (
              filteredAndSortedLeads.map((lead) => (
                <TableRow key={lead.id} className="group border-border/20 transition-all hover:bg-brand-primary/[0.03]">
                  <TableCell className="px-8 py-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold tracking-tight text-foreground">{lead.companyName}</div>
                        <div className="flex items-center gap-3 mt-1">
                          {lead.website && (
                            <a href={lead.website} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-brand-primary transition-colors">
                              <Globe size={14} />
                            </a>
                          )}
                          {lead.instagram && (
                  ...
                  <TableCell className="px-8 py-6">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest mt-0.5">{lead.email || '---'}</span>
                    </div>
                  </TableCell>
                  <TableCell className="px-8 py-6">{getStatusBadge(lead.status)}</TableCell>
                  <TableCell className="px-8 py-6">
                  ...
                  <TableCell className="px-8 py-6 text-right">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="rounded-full hover:bg-brand-primary/10 hover:text-brand-primary transition-all">
                          <DotsThreeVertical size={24} weight="bold" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-64 border-border/60 bg-background/95 backdrop-blur-xl rounded-2xl p-2 shadow-2xl">
                        <DropdownMenuLabel className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
                          {t('table.actions')}
                        </DropdownMenuLabel>
                        <DropdownMenuSeparator className="bg-border/40" />
                        
                        <DropdownMenuItem onClick={() => handleStatusUpdate(lead.id, 'CONTATO_REALIZADO')} className="rounded-xl flex items-center gap-3 p-3 cursor-pointer transition-all hover:bg-brand-primary/5">
                          <div className="size-8 rounded-lg bg-purple-500/10 flex items-center justify-center">
                            <Envelope className="text-purple-500" size={18} weight="bold" />
                          </div>
                          <span className="text-sm font-bold uppercase tracking-tight">{t('status.CONTATO_REALIZADO')}</span>
                        </DropdownMenuItem>
                        
                        <DropdownMenuItem onClick={() => handleStatusUpdate(lead.id, 'NEGOCIACAO')} className="rounded-xl flex items-center gap-3 p-3 cursor-pointer transition-all hover:bg-brand-primary/5">
                          <div className="size-8 rounded-lg bg-orange-500/10 flex items-center justify-center">
                            <Handshake className="text-orange-500" size={18} weight="bold" />
                          </div>
                          <span className="text-sm font-bold uppercase tracking-tight">{t('status.NEGOCIACAO')}</span>
                        </DropdownMenuItem>
                        
                        <DropdownMenuItem onClick={() => handleStatusUpdate(lead.id, 'CONVERTIDO')} className="rounded-xl flex items-center gap-3 p-3 cursor-pointer transition-all hover:bg-green-500/5 group/convert">
                          <div className="size-8 rounded-lg bg-green-500/10 flex items-center justify-center group-hover/convert:bg-green-500 transition-colors">
                            <CheckCircle className="text-green-500 group-hover/convert:text-white" size={18} weight="bold" />
                          </div>
                          <span className="text-sm font-black uppercase tracking-tight text-green-500">{t('status.CONVERTIDO')}</span>
                        </DropdownMenuItem>

                        <DropdownMenuSeparator className="bg-border/40" />
                        
                        <DropdownMenuItem onClick={() => handleDelete(lead.id)} className="rounded-xl flex items-center gap-3 p-3 cursor-pointer text-red-500 hover:bg-red-500/10 focus:bg-red-500/10">
                          <div className="size-8 rounded-lg bg-red-500/10 flex items-center justify-center">
                            <Trash size={18} weight="bold" />
                          </div>
                          <span className="text-sm font-bold uppercase tracking-tight">Remover Lead</span>
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
